...
for folder in ['Permanent_water', 'Reservoirs']:
    for area in ['permanent_area', 'seasonal_area']:
        ...
        save_url = save_dir / f'{start_year}_utest_a_{alpha:.1f}_p_{p_thd:.3f}.png'
        ...
        row_single = [folder, area, basin_level, start_year, alpha, p_thd, 
                      thd_low, thd_high, 'utest', neg, stable, pos, 
        ...
        if csv_init_flag & (not os.path.exists(url)): 
            df.to_csv(url, mode='w')
            csv_init_flag = False

        else: df.to_csv(url, mode='a', header=False)
